...
intrinsic MDLMFDBSearchBatchStr(table::MonStgElt, queries::SeqEnum : limit := "None") -> MonStgElt
{Runs Python lmfdb_magma.db.lmfdb_search with the give query. Returns the result as a string.}
    
    python := MDGetPythonInterpreter();
    py_src_dir := Sprintf("%o/py_src", MDLMFDBMagmaSourceDir());
    query_json := MDQueryToJSON(queries);

    // Construct the Python command
    python_cmd := Sprintf(
        "%o -c 'from lmfdb_magma import db; print(db.lmfdb_search_batch(\"%o\",%o,limit=%o))'",
        python, table, query_json, limit
    );
    full_cmd := Sprintf("cd %o && %o", py_src_dir, python_cmd);
    //print full_cmd;
...
intrinsic MDLMFDBSearchBatch(table::MonStgElt, query::SeqEnum : limit := "None") -> .
{Runs Python lmfdb_magma.db.lmfdb_search with the give query. }

    entry := MDLMFDBSearchBatchStr(table, query : limit := limit);
